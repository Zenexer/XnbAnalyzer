...
public class Texture2DReader : AsyncReader<Texture2D>
{
    public Texture2DReader(XnbStreamReader rx)
        : base(rx) { }

    public override async ValueTask<Texture2D> ReadAsync(CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < mipCount; i++)
        {
            ...
            await Rx.ReadBytesAsync(buffer, cancellationToken);

            mipImages[i] = buffer.ToImmutableArray();
        }
